...
def job(session_filenames, cell_specimen_ids, slice_after, slice_before, corrected_p_thresh, n_boot, only_spontaneous):
    datas = []
...
def output_handler(process: ParallelProcess, df_cells: pd.DataFrame, result, file_name):
    if result is None:
        return
    ...
    df_cells.to_csv(path.join(process.get_save_dir(), file_name), index_label="specimen_id")
...
if __name__ == "__main__":
    ...
    for sessions, cell_specimen_ids in sessions_to_cells.items():
        ...
        args.append((session_filenames, cell_specimen_ids, slice_after, slice_before, corrected_p_thresh, n_boot, only_spontaneous))

    df_cells = pd.DataFrame()
    process = ParallelProcess(CLASSIFIED_CELLS_SAVE_DIR)
    file_name = "cells-spontaneous.csv" if only_spontaneous else "cells"

    def output_handler_wrapper(result):
        return output_handler(process, df_cells, result, file_name)
